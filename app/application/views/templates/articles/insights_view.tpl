...
<script type="text/javascript">
    var articleId = {$response.insight->id};
    var commentsCount = {$response.insight->comments|count};
</script>
...
{block name="meta"}
{assign article $response.insight}
...
{block name="content"}
<!-- Article Content -->
{assign insight $response.insight}
{assign article $response.insight}
...
<div class="viewFullInfographicImg" ng-model="article">
    ...
    <div class="position-relative">
        <img src="{$APP_BASE_URL}image?file_name={$insight->id}/{$insight->banner_image}" alt="Infographic Image" class="infographicImgFull img-responsive">
        ...
        <div class="col-xs-5 no-padding">
            <ol class="breadcrumb">
              <li><a href="{$APP_BASE_URL}" class="text-dark-gray">Home</a></li>
               <li><a href="{$APP_BASE_URL}insights" class="text-dark-gray">Insights</a></li>
        ...
        <div class="col-xs-12 no-padding padding-vertical-50"> 
            ...
            <div class="col-xs-12 no-padding">
                ...
                <div class="col-xs-3 col-sm-1 date no-padding">
                    <div class="padding-5 background-box-grey width_90 center">
                        <p class="text-gray text-extra-large no-margin padding-5">{$insight->published_at|date_format:'M'|UPPER}</p>
                        <p class="text-gray text-24 text-bold no-margin">{$insight->published_at|date_format:'d'}</p>
                    </div>
                </div>                
                <div class="btn-group col-xs-9 col-sm-11">
                    <h1 class="text-extra-extra-large font-family-bold padding-top-20 text-metallic-blue">{$insight->title}</h1>
                </div>
            ...
            <div class="col-xs-12 no-padding margin-top-25">
                    <div class="shareButton bg-white fixedShare hidden-xs hidden-sm" fixed-share>
                        <a href="https://www.facebook.com/sharer/sharer.php?u={$APP_BASE_URL}insight/{$insight->perma_link|lower}" target="_blank"><img class="margin-3" src="{$MEDIA_FILES_URL}images/facebook.jpg"></a>
                        <div class="clearfix"></div>
                        <a href="https://plus.google.com/share?url={$APP_BASE_URL}insight/{$insight->perma_link|lower}" target="_blank"><img class="margin-3" src="{$MEDIA_FILES_URL}images/google-plus.jpg"></a>
                        <div class="clearfix"></div>
                        <a href="https://twitter.com/home?status={$APP_BASE_URL}insight/{$insight->perma_link|lower}" target="_blank"><img class="margin-3" src="{$MEDIA_FILES_URL}images/twitter.jpg"></a>
                        <div class="clearfix"></div>
                        <a href="https://www.linkedin.com/shareArticle?mini=true&url={$APP_BASE_URL}insight/{$insight->perma_link|lower}" target="_blank"><img class="margin-3" src="{$MEDIA_FILES_URL}images/linkedin.jpg"></a>
                        <div class="clearfix"></div>
                    </div>
                    <div class="col-xs-3 col-sm-1 no-padding">
                        {if !isset($insight->author_img)}
                        <img src="{$MEDIA_FILES_URL}images/Admin_new.png" class="width_90 center-margin border-2-orange radius-5">
                        {else}
                        <img src="{$insight->author_img}" class="width_90 center-margin border-2-orange radius-5">
                        {/if}
                    </div>
                    <div class="col-sm-11 col-xs-9">
                        <div class="col-xs-12 margin-top-25 no-padding">
                            {if $insight->type_id == 2}
                           <a class="infographicImg" href="#">
                                <img src="{$APP_BASE_URL}image?file_name={$insight->id}/{$insight->banner_image}" alt="Article Image" class="img-responsive width_100"/>
                                <span class="inforgaphicViewFull">
                                    <img src="{$MEDIA_FILES_URL}images/full.png">
                                </span>
                            </a>
                            {/if}
                            {if $insight->type_id == 3}
                            <span>
                                <div class= "embed-responsive embed-responsive-75  radius-3 border-bottom-light-grey">{$insight->video_link}</div>
                            </span>
                            {/if}
                        </div>
                        <div class="clearfix"></div>
                        ...
                        <div class="articleInformation margin-top-25">
                            <div class="font-family-light">{$insight->summary}</div>
                        </div>
                        <div class="clearfix"></div>
                        {if $insight->summary != ''}
                        <div class="shareButton margin-top-25 hidden-md hidden-lg">
                            <a href="https://www.facebook.com/sharer/sharer.php?u={$APP_BASE_URL}insight/{$insight->perma_link|lower}" target="_blank" class="margin-right-10">
                                <img src="{$MEDIA_FILES_URL}images/facebookButton.jpg" alt="">
                            </a>
                            <a href="https://twitter.com/home?status={$APP_BASE_URL}insight/{$insight->perma_link|lower}" target="_blank" class="margin-right-10">
