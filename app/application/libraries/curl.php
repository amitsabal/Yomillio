...
class Curl
{
    ...
    public function __construct()
    {
        ...
        $this->setHeader('X-username', (REST_USERNAME));
        $this->setHeader('X-password', (REST_PASSWORD));
        $this->setHeader('X-apikey', (REST_API_KEY));
        ...
        if( isset($_SESSION) && isset($_SESSION['session_token']))
        {
            $sessionToken = $_SESSION['session_token'];
            
            $this->setHeader('X-token', $sessionToken);
        }
    }
    ...
    public function get($url_mixed, $data = array())
    {
        if (is_array($url_mixed)) {
            ...
            foreach ($url_mixed as $url) {
                ...
                $curlURL = $this->buildURL($url, $data);
                //error_log($curlURL);
                $curl->setOpt(CURLOPT_URL, $curlURL, $curl->curl);
